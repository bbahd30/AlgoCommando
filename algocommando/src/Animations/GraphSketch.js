...
class Vertex
{
    ...
    showVertex = (p5) =>
    {
        ...
        p5.fill(this.color)
        p5.ellipse(this.xRatio * canvasWidth, this.yRatio * canvasHeight, vertexRatio, vertexRatio)
        p5.strokeWeight(2);
        p5.fill("white")
        p5.stroke("white");
    ...
    showEdge = (p5) => 
    {
        for (let vMap in this.edgeCostMap)
        {
            ...
            const [x2, y2] = [connectedV.xRatio * canvasWidth, connectedV.yRatio * canvasHeight]
            p5.line(x1, y1, x2, y2)
    ...
    while (Object.keys(verticesSet).length != 0)
    {
        ...
        for (let neighbourID in minVertex.edgeCostMap)
        {
            ...
            if (distance < neighbourVertex.distance)
            {
                ...
                await new Promise((resolve) => setTimeout(resolve, 1000))
                vertices[neighbourID].edgeColor = vertices[neighbourID].defaultEdgeColor
            }
        }
    }
